    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-18">
          ...
          {/* Right Action Buttons: Sync Status + PWA Install + Admin Portal */}
          <div className="hidden sm:flex items-center gap-2.5">
            {onQuickSync && (
              <SyncStatusBadge
                isConfigured={isBackendSynced}
                isSyncing={isSyncing}
          ...
          {onQuickSync && (
            <div className="pt-1">
              <button
                ...
                disabled={isSyncing}
                className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-100"
              >
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${isBackendSynced ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`} />
